...
/*   Updated: 2025/05/05 17:38:52 by selbouka         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_environ_list	*empty_environ(t_environ_list *env)
{
	...
	shlvl = ft_strdup1("SHLVL=1");
	PATH = ft_strdup1("PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin");
	mini = ft_strdup1("_=/home/asebban/Desktop/shell_with_selbouka/./minishell");
...
t_environ_list	*init_environ(char **envp)
{
	...
	env = (t_environ_list *)malloc(sizeof(t_environ_list)); // 2
	if (!env)
		return (NULL);
	if (!*envp)
...
t_shell	*init_shell(char **env)
{
	...
	if (!shell->env)
		return (free_environ(shell), NULL);
	shell->lex_head = NULL;
	shell->executor = NULL;
	return (shell);
}
